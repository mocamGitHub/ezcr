-- ========================================
-- SEED CONFIGURATOR DATA FOR DEV TENANT
-- ========================================
-- This migration seeds configurator data for the ezcr-dev tenant
-- Based on migration 00009 but updated for development environment
...
BEGIN
  ...
  IF ezcr_tenant_id IS NULL THEN
    RAISE EXCEPTION 'ezcr-dev tenant not found. Run migration 00013_add_dev_tenant.sql first.';
  END IF;
  ...
  RAISE NOTICE 'Successfully seeded configurator data for ezcr-dev tenant (%)!', ezcr_tenant_id;
  RAISE NOTICE 'Populated:';
  RAISE NOTICE '  - 12 measurement ranges';
  RAISE NOTICE '  - 2 models, 4 extensions, 2 delivery, 3 services, 2 boltless, 4 tiedown';
  RAISE NOTICE '  - 8 business rules';
  RAISE NOTICE '  - 4 general settings';
