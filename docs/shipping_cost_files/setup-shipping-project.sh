...
# ============================================
#
# This script creates the folder structure and
# provides instructions for setting up the
# shipping integration project for Claude Code.
#
# Usage:
...
# Project name
PROJECT_NAME="ezcycleramp-shipping"

echo ""
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}  EZ Cycle Ramp - Shipping Integration     ${NC}"
echo -e "${BLUE}  Project Setup Script                     ${NC}"
echo -e "${BLUE}============================================${NC}"
echo ""

# Ask for project location
read -p "Enter project directory path (default: ./${PROJECT_NAME}): " PROJECT_DIR
PROJECT_DIR=${PROJECT_DIR:-"./${PROJECT_NAME}"}

echo ""
echo -e "${YELLOW}Creating project structure at: ${PROJECT_DIR}${NC}"
echo ""

# Create directory structure
mkdir -p "${PROJECT_DIR}/docs"
mkdir -p "${PROJECT_DIR}/supabase/migrations"
mkdir -p "${PROJECT_DIR}/supabase/functions/get-shipping-quote"
mkdir -p "${PROJECT_DIR}/supabase/functions/stripe-webhook"
mkdir -p "${PROJECT_DIR}/supabase/functions/trigger-post-purchase-emails"
mkdir -p "${PROJECT_DIR}/src/components/shipping"
...
cat > "${PROJECT_DIR}/supabase/migrations/README.md" << 'EOF'
# Database Migrations
...
cat > "${PROJECT_DIR}/supabase/functions/get-shipping-quote/index.ts" << 'EOF'
// Download supabase-get-shipping-quote.ts from Claude outputs
// and replace this file with its contents
EOF

cat > "${PROJECT_DIR}/supabase/functions/stripe-webhook/index.ts" << 'EOF'
// Download supabase-stripe-webhook.ts from Claude outputs
// and replace this file with its contents
EOF

cat > "${PROJECT_DIR}/supabase/functions/trigger-post-purchase-emails/index.ts" << 'EOF'
// Download supabase-post-purchase-emails.ts from Claude outputs
// and replace this file with its contents
EOF

cat > "${PROJECT_DIR}/src/components/shipping/README.md" << 'EOF'
# Shipping Components
...
cat > "${PROJECT_DIR}/src/hooks/README.md" << 'EOF'
# Hooks
...
cat > "${PROJECT_DIR}/src/types/shipping.ts" << 'EOF'
...
cat > "${PROJECT_DIR}/n8n-workflows/README.md" << 'EOF'
# n8n Workflows
...
# Create Claude Code starter prompt file
cat > "${PROJECT_DIR}/CLAUDE-CODE-START.md" << 'EOF'
...
# Create file download checklist
cat > "${PROJECT_DIR}/DOWNLOAD-CHECKLIST.md" << 'EOF'
...
# Test data (don't commit to prod)
...
echo ""
find "${PROJECT_DIR}" -type f -name "*.md" -o -name "*.ts" -o -name "*.example" -o -name ".gitignore" 2>/dev/null | sort | head -20
echo ""
echo -e "${YELLOW}Next Steps:${NC}"
echo ""
echo "  1. Download files from Claude outputs"
echo "     See: ${PROJECT_DIR}/DOWNLOAD-CHECKLIST.md"
echo ""
echo "  2. Place files in correct locations"
...
echo "     claude"
echo ""
echo "  5. Paste the starter prompt"
echo "     See: ${PROJECT_DIR}/CLAUDE-CODE-START.md"
echo ""
echo -e "${GREEN}Good luck with your shipping integration!${NC}"
echo ""
