// ============================================
// SUPABASE EDGE FUNCTION: get-shipping-quote
// T-Force Freight LTL Rating Integration
// ============================================
// 
// Deploy: supabase functions deploy get-shipping-quote
// 
// Required Secrets:
//   supabase secrets set TFORCE_CLIENT_ID=your_client_id
//   supabase secrets set TFORCE_CLIENT_SECRET=your_client_secret
...
async function getAccessToken(): Promise<string> {
  const clientId = Deno.env.get('TFORCE_CLIENT_ID')
  const clientSecret = Deno.env.get('TFORCE_CLIENT_SECRET')
...
serve(async (req) => {
  ...
  try {
    ...
    // Get request for logging (may fail if body already consumed)
    let request: ShippingQuoteRequest | undefined
    try {
      request = await req.json()
    } catch {
      // Body already consumed, create minimal request
      request = {
