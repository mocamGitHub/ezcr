...
function ConfidenceMeter({ level, showUpgrade = false, onUpgrade }) {
  const levels = {
  ...
  return (
    <div className="bg-zinc-800/50 rounded-lg p-4">
      <div className="flex items-center justify-between mb-2">
        <span className={`text-${current.color}-500 font-medium`}>{current.label}</span>
        <span className="text-zinc-500 text-sm">{current.percent}%</span>
      </div>
      
      <div className="h-2 bg-zinc-700 rounded-full overflow-hidden mb-3">
        <div 
          className={`h-full bg-${current.color}-500 rounded-full transition-all duration-700`}
...
function MeasurementUpgrade({ onComplete, onSkip }) {
...
export default function Option3DHybridConfidence() {
  ...
  const [confidenceLevel, setConfidenceLevel] = useState('likely');
