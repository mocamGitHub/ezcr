...
interface ConversationStep {
  ...
  type: 'bot'
  message?: string
  getMessage?: (answers: Record<string, string>) => string | null
...
function BotMessage({ message, isNew }: BotMessageProps) {
  return (
    <div
...
function QuickReplies({ options, onSelect, disabled }: QuickRepliesProps) {
  return (
    <div
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        <div className="max-w-md mx-auto space-y-4">
          {messages.map((msg, index) => (
            <div key={index}>
              {msg.type === 'bot' && (
                <BotMessage message={msg.content as string} isNew={index === messages.length - 1} />
              )}
              {msg.type === 'user' && (
                <UserMessage message={msg.content as string} />
              )}
              {msg.type === 'result' && (
                <ResultCard result={msg.content as Result} />
              )}
            </div>
          ))}
