...
const TonneauIllustrations: Record<string, React.ReactNode> = {
  'roll-up-soft': (
...
interface RollDirectionOptionProps {
  direction: 'on-top' | 'into-bed'
  selected: boolean
  onClick: () => void
}

function RollDirectionOption({ direction, selected, onClick }: RollDirectionOptionProps) {
...
function CompatibilityPanel({ answers }: CompatibilityPanelProps) {
  const items: { label: string; complete: boolean }[] = []

  if (answers.bedLength) {
    const labels: Record<string, string> = { short: "Short bed (5'-5.8')", standard: "Standard (6'-6.5')", long: "Long (8'+)", unsure: "Bed length: Unknown" }
  ...
  if (answers.bikeWeight) {
    const labels: Record<string, string> = { light: 'Under 500 lbs', medium: '500-800 lbs', heavy: '800-1,200 lbs', over: 'Over 1,200 lbs' }
...
export default function Option2BVisualTonneau() {
  ...
  const getActiveSteps = () => {
    const active = ['bedLength', 'hasTonneau']
  ...
  const handleAnswer = (value: string) => {
    ...
    const newActiveSteps = ['bedLength', 'hasTonneau']
  ...
  const calculateRecommendation = (ans: Record<string, string>): Result => {
