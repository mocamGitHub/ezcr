...
interface TruckBedVisualProps {
  bedLength: string
  tonneauType: string
  rollDirection: string | null
}

function TruckBedVisual({ bedLength, tonneauType, rollDirection }: TruckBedVisualProps) {
  const bedWidths: Record<string, number> = { short: 100, standard: 130, long: 170 }
...
// Bed length selector
interface BedLengthSelectorProps {
  value: string
  onChange: (value: string) => void
}

function BedLengthSelector({ value, onChange }: BedLengthSelectorProps) {
  const options = [
...
interface TonneauSelectorProps {
  value: string | null
  onChange: (value: string) => void
}

function TonneauSelector({ value, onChange }: TonneauSelectorProps) {
  return (
    <div>
...
interface RollDirectionSelectorProps {
  value: string | null
  onChange: (value: string) => void
}

function RollDirectionSelector({ value, onChange }: RollDirectionSelectorProps) {
  return (
...
function CompatibilityStatus({ bedLength, tonneauType, rollDirection }: CompatibilityStatusProps) {
  const issues: string[] = []
  const notes: string[] = []
