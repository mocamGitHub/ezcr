...
function calculateQualification(answers: Record<string, string>): 'high' | 'medium' | 'low' {
  let score = 0
...
function CalendlyEmbed({ onScheduled }: { onScheduled: (data: BookingData) => void }) {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault()
    onScheduled({ ...formData, date: selectedDate || undefined, time: selectedTime || undefined })
  }
...
function CallbackForm({ onSubmit }: { onSubmit: (data: CallbackFormData) => void }) {
...
// Confirmation screen
function BookingConfirmation({ booking }: { booking: BookingData }) {
  return (
...
function SelfServiceResult({ onReset }: { onReset: () => void }) {
  return (
    <div className="text-center">
...
export default function Option3EConsultantApproach() {
  ...
  const handleScheduled = (data: BookingData) => {
    setBooking(data)
  }
