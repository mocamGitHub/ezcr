...
//   SENDGRID_TEMPLATE_OUT_FOR_DELIVERY=d-xxx
...
async function sendEmail(
  ...
  const templateId = process.env[template.templateId];
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (request.type === 'process_scheduled') {
      ...
      return NextResponse.json({ success: true, message: 'Scheduled emails processed', ...result });
    }
    ...
    switch (request.type) {
      ...
      default:
        return NextResponse.json(
          { error: `Unknown email type: ${request.type}` },
          { status: 400 }
        );
    }
